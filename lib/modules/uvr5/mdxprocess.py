...
model_ids_demucs_inpure = [
    name.split(":")[1].strip() for name in demucs_download_list.keys()
]
...
def id_to_ptm(mkey):
    if mkey in model_ids:
        ...
        if not os.path.exists(f"{now_dir}/assets/uvr5_weights/MDX/{mkey}"):
            ...
            subprocess.run(["python", "-m", "wget", "-o", mpath, _Models + mkey])
...
def run_mdx(
    ...
    save_path = os.path.join("assets", "audios", save_path)
    ...
    if diff:
        ...
        save_path = os.path.join("assets", "audios", "audio-others", save_path)
